...
export class ApicallserviceService {
  ...
  public ApiCall(...queryCall): Observable<any> {
    this.master=arguments[1].split('/')
   if(this.master[0]==='mstserv' || this.master[0]==='usrserv'||  this.master[0]==='venserv'){
      this.Url=environment.kvbbeURL
   }
   else {
this.Url=environment.apiURL
    ...
    if (arguments[0] == 'post') {
      return this.http.post(this.Url + arrangeAPI, arguments[2], { headers: headers });
    }

    if (arguments[0] == 'get') {
      return this.http.get(this.Url + arrangeAPI, { headers: headers });
    }

    if (arguments[0] == 'getFile') {
      arrangeAPI = arguments[0]+arguments[1] 
      return this.http.get(this.Url + arrangeAPI, { headers, responseType: 'blob' as 'json' });
    }
  }
